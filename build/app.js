...
var require_library = __commonJS({
  "generated/prisma/runtime/library.js"(exports2, module2) {
    "use strict";
...
`), o = o.replace(/\\r/g, "\r")), r[i] = o;
    ...
    var Vf = {};
    tr(Vf, { DMMF: () => ut, Debug: () => N, Decimal: () => ge, Extensions: () => ri, MetricsClient: () => Lr, PrismaClientInitializationError: () => T, PrismaClientKnownRequestError: () => z3, PrismaClientRustPanicError: () => le, PrismaClientUnknownRequestError: () => j, PrismaClientValidationError: () => Z, Public: () => ti, Sql: () => oe, createParam: () => Ra, defineDmmfProperty: () => Da, deserializeJsonResponse: () => Tr, deserializeRawResult: () => zn, dmmfToRuntimeDataModel: () => Zs, empty: () => La, getPrismaClient: () => gu, getRuntime: () => Vn, join: () => Na, makeStrictEnum: () => hu, makeTypedQueryFactory: () => _a, objectEnumValues: () => Cn, raw: () => eo, serializeJsonQuery: () => Nn, skip: () => _n, sqltag: () => ro, warnEnvConflicts: () => yu, warnOnce: () => st });
    ...
    globalThis.DEBUG ?? (globalThis.DEBUG = ii.DEBUG ?? "");
    globalThis.DEBUG_COLORS ?? (globalThis.DEBUG_COLORS = ii.DEBUG_COLORS ? ii.DEBUG_COLORS === "true" : true);
    var Yr = { enable(e) {
...
`);
    ...
    var On = class {
      constructor(r, t) {
        __publicField(this, "computedFieldsCache", new Pe());
        __publicField(this, "modelExtensionsCache", new Pe());
        __publicField(this, "queryCallbacksCache", new Pe());
    ...
    function $r({ error: e, user_facing_error: r }, t, n) {
      return r.error_code ? new z3(cm(r, n), { code: r.error_code, clientVersion: t, meta: r.meta, batchRequestIdx: r.batch_request_idx }) : new j(e, { clientVersion: t, batchRequestIdx: r.batch_request_idx });
    }
    function cm(e, r) {
      let t = e.message;
...
`);
    ...
    function Ao(e) {
      return function(t, n) {
        let i, o = (s = e) => {
          try {
            return s === void 0 || s?.kind === "itx" ? i ?? (i = Hl(t(s))) : Hl(t(s));
          } catch (a) {
    ...
    function gu(e) {
      class r {
        constructor(n) {
          ...
          __publicField(this, "_middlewares", new Hn());
...
var require_prisma = __commonJS({
  "generated/prisma/index.js"(exports2) {
    "use strict";
...
async function userRoutes(app2) {
  app2.post("/create", async (req, reply) => {
    const result = userSchema.safeParse(req.body);
    if (!result.success) {
      return reply.status(400).send({ message: "Erro ao criar um usu\xE1rio" });
    }
    ...
    if (verifyIfUserExists) {
      return reply.status(400).send({ message: "O email que voc\xEA est\xE1 tentando cadastrar j\xE1 existe." });
    }
    ...
    try {
      ...
      return reply.status(201).send({ message: "Usu\xE1rio cadastrado com sucesso." });
    } catch (error) {
      console.log("Erro ao criar um usu\xE1rio", error);
      return reply.status(500).send({ message: "Erro ao criar um usu\xE1rio" });
    }
  });
  app2.get("/list", async (req, reply) => {
    try {
      ...
      console.log("Erro ao listar os usu\xE1rios", error);
      return reply.status(500).send({ message: "Erro ao listar os usu\xE1rios" });
    }
  });
  app2.get("/get/:id", async (req, reply) => {
    const paramsSchema = import_zod2.z.object({
      id: import_zod2.z.string().uuid("O id informado n\xE3o \xE9 um uuid v\xE1lido")
    });
    const result = paramsSchema.safeParse(req.params);
    if (!result.success) {
      console.log("Erro ao buscar o usu\xE1rio", result.error);
      return reply.status(400).send({ message: "Erro ao buscar o usu\xE1rio" });
    }
    try {
      ...
      console.log("Erro ao buscar o usu\xE1rio", error);
      return reply.status(500).send({ message: "Erro ao buscar o usu\xE1rio" });
    }
  });
  app2.put("/update/:id", async (req, reply) => {
    const paramsSchema = import_zod2.z.object({
      id: import_zod2.z.string().uuid("O id informado n\xE3o \xE9 um uuid v\xE1lido")
    });
    ...
    if (!resultParams.success || !resultBody.success) {
      console.log("Erro ao tentar atualizar o usu\xE1rio", resultParams.error, resultBody.error);
      return reply.status(400).send({ message: "Erro ao tentar atualizar o usu\xE1rio, tente novamente!" });
    }
    const { id } = resultParams.data;
    const { name, email, password } = resultBody.data;
    const passwordHash = password ? await (0, import_bcryptjs.hash)(password, 10) : void 0;
    try {
      ...
      console.log("Erro ao tentar atualizar o usu\xE1rio", error);
      return reply.status(500).send({ message: "Erro ao tentar atualizar o usu\xE1rio, tente novamente!" });
    }
  });
  app2.delete("/delete/:id", async (req, reply) => {
    const paramsSchema = import_zod2.z.object({
      id: import_zod2.z.string().uuid("O id informado n\xE3o \xE9 um uuid v\xE1lido")
    });
    const result = paramsSchema.safeParse(req.params);
    if (!result.success) {
      console.log("Erro ao tentar deletar o usu\xE1rio", result.error);
      return reply.status(400).send({ message: "Erro ao tentar deletar o usu\xE1rio, tente novamente!" });
    }
    try {
      ...
      console.log("Erro ao tentar deletar o usu\xE1rio", error);
      return reply.status(500).send({ message: "Erro ao tentar deletar o usu\xE1rio, tente novamente!" });
    }
  });
}
...
app.listen({
  port: env.PORT
}).then(() => console.log("Server is running on port 3333 \u{1F680}\u{1F3C6}"));
